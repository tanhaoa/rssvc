...
单文件 exe，无运行时依赖，release 构建约 **500 KB**，还内置了一个原生 Win32
...
- **状态自动刷新**：每 1.5 秒轮询一次；操作按钮在执行期间自动禁用，
  操作在后台线程执行，界面不会卡住
- **服务详情**：选中任一服务，下方面板显示启动参数、工作目录、日志路径、
  轮转与重启策略、优先级、运行账户等完整配置
- **一键操作**：启动 / 停止 / 重启 / 暂停 / 继续 / 删除服务（删除需确认）
- **编辑配置**：点击"编辑配置"弹出表单，预填当前全部参数：显示名称、描述、
  应用程序、参数、工作目录、日志路径、轮转、启动类型、优先级、重启策略、
  停止超时、环境变量均可修改并保存；保存后自动下发 PARAM_CHANGE（轮转参数
  即时生效），并可选立即重启服务使全部配置生效（后台执行，不卡界面）
...
- **图形化安装**：点击"安装新服务"弹出表单，支持文件/目录浏览对话框，
  可填写服务名、显示名、程序、参数、目录、stdout/stderr、启动类型与优先级，
  与 `rssvc install` 行为完全一致
